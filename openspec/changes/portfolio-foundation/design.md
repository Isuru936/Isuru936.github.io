...
**React Three Fiber over imperative three.js.** R3F owns the render loop and, more importantly, disposal — the main source of GPU context leaks across client-side navigations. It also lets scene state be driven by ordinary React state. *Trade-off:* a port cost if the Claude Design source turns out to be raw three.js.
